...
onedataset_sim = function(k0, basisfun_truemod = TRUE , stationary_x = TRUE){
  ...
  if (basisfun_truemod) {
    ...
    #spranef <- RFsimulate(model = RMmatern(nu = 0.5, var = 1, scale = 1), x = xy$x, y = xy$y, n = 1)@data[,1] 
    ...
    # u = (xy$x + 0.5*sqrt(n))/sqrt(n)
    # v = (xy$y + 0.5*sqrt(n))/sqrt(n)
    #spranef = 0.5*(1.35 + exp(u)*13*(u - 0.6)^2*exp(-v)*sin(7*v))
  ...
  for (j in 1:n) {
    score_x = x_basisfunctions[j,]*(W[j] - X_pred[j])/phi_hat
    ...
    score = c(score_x,score_y0,score_y1,score_y_rho)
    varscore = varscore + tcrossprod(score)
  }
  ...
  varscore[-(1:2),-(1:2)] = varscore[-(1:2),-(1:2)] + bdiag.spam(ginv(sigmarho_hat),ginv(sigmax_hat))
  ...
  out <- list(fit_naive = naive_mod, fit_correct = fit, no.spatial_fit = no.spatial_mod, report = report,
              varscore = varscore, jointPrecision = jointPrecision)
  return(out)
}
...
for (k0 in 1:num_sims) {
  ...
  message(paste0("Finished Datset ",k0))
}
