...
#gdbsource("norm2step_SE.R",interactive=TRUE)
TMB::compile("normal_2step.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("normal_2step"))

true_coefs = c(1,1)
n = 1024
basisfun_truemod = FALSE
stationary_x = TRUE
...
onedataset_sim = function(k0){
  ...
  if (basisfun_truemod) {
    ...
    alpha_x <- rmvnorm(n = 1, mu = rep(0,ncol(x_basisfun_true)), Sigma = diag(nrow = ncol(x_basisfun_true))) %>%
      as.vector
    
    X <- x_basisfun_true%*%alpha_x
    
    alpha_rho <- rmvnorm(n = 1, mu = rep(0,ncol(rho_basisfun_true)), Sigma = diag(nrow = ncol(rho_basisfun_true))) %>%
      as.vector
    ...
    spranef <- RFsimulate(model = RMmatern(nu = 0.5, var = 2, scale = 0.05), x = xy$x, y = xy$y, n = 1)@data[,1] 
  ...
  sigma_y = 1
  simy <- rnorm(n, mean = eta, sd = sigma_y)
  ...
  naive_mod = glm(simy ~ W + sp_basisfunctions)
  no.spatial_mod = glm(simy ~ X_pred)
  ...
  obj <- MakeADFun(data=inputdata,DLL = "normal_2step",
  ...
  ###adjustment terms for the penalty term
