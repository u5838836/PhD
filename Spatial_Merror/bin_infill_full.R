rm(list=ls())
library(TMB)
library(mvtnorm)
library(autoFRK)
library(mgcv)
library(spaMM)
#library(tidyverse)
library(RandomFields)
library(sp)
library(ggplot2)
library(tidyr)
library(forcats)
library(fields)
library(optimx)
#library(FRK)
library(Matrix)
library(MASS)
library(dplyr)
library(parallel)
library(foreach)
library(doParallel)
library(glm2)
library(xtable)

cl <- makeCluster(detectCores()-1)
registerDoParallel(cl)


setwd("D:/Study new/PhD/R code/Spatial_Merror")
#gdbsource("poisson2D_2step.R",interactive=TRUE)
TMB::compile("multX_bin.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("multX_bin"))


#autoFRK basis functions for the true model

# x_basisfun_true <- mrts(xy, k = 10, x = xy) %>% 
#   as.matrix
# 
# x2_basisfun_true <- mrts(xy, k = 20, x = xy) %>% 
#   as.matrix
# 
# rho_basisfun_true <- mrts(xy, k = 5, x = xy) %>%
#   as.matrix
# rho_basisfun_true <- rho_basisfun_true[,-1]

#Construct reduced-rank Sigma_x and Sigma_rho, with nugget effect

#set.seed(24)

# sigma_x = rWishart(1,ncol(x_basisfun_true),Sigma = diag(1/(1:ncol(x_basisfun_true)))/ncol(x_basisfun_true))[,,1]
# sigma_x = 5*sigma_x/ncol(x_basisfun_true)
# sigma_x = diag(1/(1:ncol(x_basisfun_true)))
# 
# sigma_x2 = rWishart(1,ncol(x2_basisfun_true),Sigma = diag(1/(1:ncol(x2_basisfun_true)))/ncol(x2_basisfun_true))[,,1]
# sigma_x2 = 5*sigma_x2/ncol(x2_basisfun_true)
# sigma_x2 = diag(1/(1:ncol(x2_basisfun_true)))


# gamma_rho = t(chol((rWishart(1,ncol(rho_basisfun_true), Sigma = diag(1/(1:ncol(rho_basisfun_true)))/
#                             ncol(rho_basisfun_true)))[,,1]))[1:ncol(rho_basisfun_true),1:reduced_rank]
# sigma_rho = tcrossprod(gamma_rho) + 0.2*diag(ncol(rho_basisfun_true))
# sigma_rho = 5*sigma_rho/ncol(rho_basisfun_true)

# sigma_rho = rWishart(1,ncol(rho_basisfun_true), Sigma = diag(1/(1:ncol(rho_basisfun_true)))/
#                        ncol(rho_basisfun_true))[,,1]
# sigma_rho = 5*sigma_rho/ncol(rho_basisfun_true)
# sigma_rho = diag(1/(1:ncol(rho_basisfun_true)))


# gamma_x = matrix(0,ncol = ncol(x_basisfun_true), nrow = ncol(x_basisfun_true))
# gamma_x[lower.tri(gamma_x)] = rnorm(sum(lower.tri(gamma_x)))
# diag(gamma_x) = 1/(1:ncol(x_basisfun_true))
# sigma_x = tcrossprod(gamma_x) + diag(1/(1:ncol(x_basisfun_true)))
# sigma_x = sigma_x/600*max(sigma_x)
# 
# gamma_rho = matrix(0,ncol = reduced_rank, nrow = ncol(rho_basisfun_true))
# gamma_rho[lower.tri(gamma_rho)] = rnorm(sum(lower.tri(gamma_rho)))
# diag(gamma_rho) = 1/(1:reduced_rank)
# sigma_rho = tcrossprod(gamma_rho) + diag(1/(1:ncol(rho_basisfun_true)))
# sigma_rho = sigma_rho/400*max(sigma_rho)


onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = FALSE){
  
  #set.seed(73*k0)
  
  #Dataset generation
  
  ## Generate covariates and spatial random effects
  
  
  if (basisfun_truemod) {
    
    #set.seed(23)
    
    alpha_x <- rmvnorm(n = 1, mu = rep(0,ncol(x_basisfun_true)), Sigma = sigma_x) %>%
      as.vector
    
    X <- x_basisfun_true%*%alpha_x
    
    
    alpha_x2 <- rmvnorm(n = 1, mu = rep(0,ncol(x2_basisfun_true)), Sigma = sigma_x2) %>%
      as.vector
    
    X2 <- x2_basisfun_true%*%alpha_x2
    
    
    alpha_rho <- rmvnorm(n = 1, mu = rep(0,ncol(rho_basisfun_true)), Sigma = sigma_rho) %>%
      as.vector
    
    spranef <- rho_basisfun_true%*%alpha_rho
    
    #set.seed(NULL)
    
  } else {
    
    if (stationary_x) {
      
      #set.seed(23)
      
      X <- RFsimulate(model = RMmatern(nu = 1, var = 0.5, scale = 5), x = xy$x, y = xy$y, n = 1)@data[,1]
      
      X2 <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 5), x = xy$x, y = xy$y, n = 1)@data[,1]
      
      #set.seed(NULL)
      
      # set.seed(23)
      # 
      # alpha_x <- rmvnorm(n = 1, mu = rep(0,ncol(x_basisfun_true)), Sigma = sigma_x) %>%
      #   as.vector
      # 
      # X <- x_basisfun_true%*%alpha_x
      # 
      # set.seed(NULL)
      
    } else {
      
      1
      
      ######-------A few different possible ways to generate (non-stationary) X-------######
      
      # w_1 <- rnorm(1,sd=1)
      # w_2 <- rnorm(1,sd=0.5)
      # 
      # f_1 <- apply(xy, 1, function(x){cos(pi*norm(x-c(0,1),type = "F")) })
      # f_2 <- apply(xy, 1, function(x){cos(2*pi*norm(x-c(3/4,1/4),type = "F")) })
      # 
      # X <- (f_1*w_1 + f_2*w_2)
      
      
      
      # X <- RFsimulate(model = RMmatern(nu = 1, var = 0.5, scale = 5), x = xy$x, y = xy$y, n = 1)@data[,1] +
      #   0.01*xy$x + 0.01*xy$y
      
      # X <- RFsimulate(model = RMmatern(nu = 1, var = 1, scale = 0.3), x = xy$x, y = xy$y, n = 1)@data[,1] +
      #   seq(0,1,length.out = 1024)
      
      # set.seed(23)
      # 
      # X2 <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 5), x = xy$x, y = xy$y, n = 1)@data[,1]
      # 
      # set.seed(NULL)
      # 
      # X <- 2*exp(-(xy$x^2+xy$y^2)/sqrt(n))
      
      # u = (xy$x + 0.5*sqrt(n))/sqrt(n)
      # v = (xy$y + 0.5*sqrt(n))/sqrt(n)
      # X = 0.5*(1.35 + exp(u)*13*(u - 0.6)^2*exp(-v)*sin(7*v))
      #X = u*sin(4*pi*v) + 1
      # X = 2*(pi**0.3*0.4)*(1.2*exp(-(u-0.2)^2/0.3^2-(v-0.3)^2/0.4^2)+
      #                0.8*exp(-(u-0.7)^2/0.3^2-(v-0.8)^2/0.4^2))
      
      
      # u = 10*xy$x/sqrt(n)
      # v = 10*xy$y/sqrt(n)
      # X = 5*exp( -2.5*((u-2)^2 + (v-2)^2 ) ) + exp(-(u^2 + v^2)/3)
      
      
      # set.seed(23)
      # 
      # alpha_x <- rmvnorm(n = 1, mu = rep(0,ncol(x_basisfun_true)), Sigma = sigma_x) %>%
      #   as.vector
      # 
      # X <- x_basisfun_true%*%alpha_x
      # 
      # set.seed(NULL)
    }
    
    # set.seed(23)
    # 
    # spranef <- RFsimulate(model = RMmatern(nu = 0.5, var = 0.5, scale = 3), x = xy$x, y = xy$y, n = 1)@data[,1] 
    # 
    # set.seed(NULL)
    
    #spranef <- rep(0,n)
    
    # set.seed(2345)
    #  
    # alpha_rho <- rmvnorm(n = 1, mu = rep(0,ncol(rho_basisfun_true)), Sigma = sigma_rho) %>%
    #   as.vector
    # 
    # spranef <- rho_basisfun_true%*%alpha_rho
    # 
    # set.seed(NULL)
    
    #spranef <- 2*exp(-(xy$x^2+xy$y^2)/sqrt(n))
    
    # u = xy$x
    # v = xy$y
    
    # u = (xy$x + 0.5*sqrt(n))/sqrt(n)
    # v = (xy$y + 0.5*sqrt(n))/sqrt(n)
    # 
    # spranef = 5*(exp(u)*(u - 0.6)^2*exp(-v)*sin(7*v))
    
    #spranef = u*sin(4*pi*v)
    # spranef = 2*(pi**0.3*0.4)*(1.2*exp(-(u-0.2)^2/0.3^2-(v-0.3)^2/0.4^2)+
    #                0.8*exp(-(u-0.7)^2/0.3^2-(v-0.8)^2/0.4^2))
    
    1
  }
  
  #sigma_e = sqrt(0.1)
  W <- X + rnorm(n, sd = sigma_e/2)
  W2 <- X2 + rnorm(n, sd = sigma_e/1.5)
  
  #Fit the model for W via autoFRK()
  fit_Wmodel <- autoFRK(Data = matrix(W,ncol=1), loc = xy, G = x_basisfunctions)
  X_pred <- predict(fit_Wmodel)$pred
  
  fit_W2model <- autoFRK(Data = matrix(W2,ncol=1), loc = xy, G = x2_basisfunctions)
  X2_pred <- predict(fit_W2model)$pred
  
  ## Generate response
  
  sizes = rep(1,n)
  eta <- cbind(1,X, X2) %*% true_coefs + spranef
  simy <- rbinom(n, sizes, binomial()$linkinv(eta))
  
  
  #Get some 'reasonable' starting values
  
  naive_mod = glm(cbind(simy,sizes-simy) ~ W + W2 + sp_basisfunctions , family = binomial)
  no.spatial_mod = glm(cbind(simy,sizes-simy) ~ X_pred + X2_pred , family = binomial)
  dnaive_mod = glm(cbind(simy,sizes-simy) ~ W + W2 , family = binomial)
  
  cholSigma = diag(log(abs(naive_mod$coefficients[-(1:3)])))[,1:reduced_rank]
  cholSigma_vec = cholSigma[lower.tri(cholSigma, diag = TRUE)]
  #sigma_nugget_sq = 0.02
  
  inputdata=list(sp_basis = sp_basisfunctions , y = as.matrix(simy), predX = X_pred, predX2 = X2_pred,
                 true_rank = reduced_rank, identity_matrix = diag(nrow=ncol(sp_basisfunctions)), sizes = sizes) 
  parameterlist=list(beta = naive_mod$coefficients[1:3] , cholSigma = cholSigma_vec , 
                     alpha_rho = naive_mod$coefficients[-(1:3)]) # , sigma_nugget_sq = sigma_nugget_sq 
  
  
  obj <- MakeADFun(data=inputdata,DLL = "multX_bin",
                   parameters = parameterlist, random = c("alpha_rho"), silent=T)
  
  
  # fit <- optimr(par = obj$par, fn = obj$fn, gr = obj$gr, method = "BFGS", hessian = TRUE,
  #                       control=list(maxit=500, trace=0))
  
  fit <- nlminb(start = obj$par, objective = obj$fn, gradient = obj$gr,
                control=list(trace = 0, iter.max = 1000, eval.max = 1000))
  
  
  ###-------Get SEs--------###
  
  #Examine outputted SEs from TMB; *should* be correct when model is correct and true X is used
  
  report = sdreport(obj,getJointPrecision = TRUE, getReportCovariance = FALSE, skip.delta.method = TRUE)
  report_summary=summary(report)
  params = report_summary %>% 
    as.data.frame %>% 
    filter(rownames(report_summary)%in%c("alpha_rho","beta")) %>%
    select("Estimate") %>% .$Estimate
  
  sigmarho_hat = as.list(report,"Est",TRUE)$Sigma
  
  #Non-parametric estimate of variance of score
  
  sigmax_hat <- fit_Wmodel$M
  alphax_hat <- fit_Wmodel$w
  phi_hat <- fit_Wmodel$s
  
  sigmax2_hat <- fit_W2model$M
  alphax2_hat <- fit_W2model$w
  phi_hat2 <- fit_W2model$s
  
  #crossprod(x_basisfunctions,W - X_pred)/phi_hat - ginv(sigmax_hat)%*%alphax_hat
  #obj$gr(fit$par)
  varscore_x = 0
  varscore_x2 = 0
  varscore_y = 0
  
  eta = params[1] + params[2]*X_pred + params[3]*X2_pred + sp_basisfunctions%*%params[-(1:3)] 
  iterative_weights = sizes*binomial()$var(binomial()$linkinv(eta)) %>% as.vector %>% diag.spam
  
  for (j in 1:n) {
    varscore_x = varscore_x + tcrossprod(x_basisfunctions[j,]*(W[j] - X_pred[j])/phi_hat ) # - ginv(sigmax_hat)%*%alphax_hat/n
    
    varscore_x2 = varscore_x2 + tcrossprod(x2_basisfunctions[j,]*(W2[j] - X2_pred[j])/phi_hat2 )
    
    score_y0 = simy[j] - sizes[j]*binomial()$linkinv(eta[j]) #-
    # 0.5*sum(diag(solve(crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions +
    #             ginv(sigmarho_hat))%*%
    # crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions))/n
    
    score_y1 = X_pred[j]*(simy[j] - sizes[j]*binomial()$linkinv(eta[j])) #- 
    # 0.5*sum(diag(solve(crossprod(sp_basisfunctions,diag.spam(as.vector(X_pred))%*%iterative_weights)%*%
    #                      sp_basisfunctions + ginv(sigmarho_hat))%*%
    #                crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions))/n
    
    score_y2 = X2_pred[j]*(simy[j] - sizes[j]*binomial()$linkinv(eta[j]))
    
    score_y_rho = sp_basisfunctions[j,]*(simy[j] - sizes[j]*binomial()$linkinv(eta[j])) # - ginv(sigmarho_hat)%*%params[-(1:2)]/n
    score_y = c(score_y0,score_y1,score_y2,score_y_rho)
    varscore_y = varscore_y + tcrossprod(score_y)
  }
  
  ###adjustment terms for the log determinant and penalty term
  
  # varscore_y[1:2,1:2] = varscore_y[1:2,1:2] +
  #   tcrossprod(c( -0.5*sum(diag(solve(crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions +
  #                                     ginv(sigmarho_hat))%*%
  #                               crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions)) ,
  #               - 0.5*sum(diag(solve(crossprod(sp_basisfunctions,diag.spam(as.vector(X_pred))%*%iterative_weights)%*%
  #                                      sp_basisfunctions + ginv(sigmarho_hat))%*%
  #                                crossprod(sp_basisfunctions,iterative_weights)%*%sp_basisfunctions))))
  
  
  varscore_y[-(1:3),-(1:3)] = varscore_y[-(1:3),-(1:3)] + ginv(sigmarho_hat,1*(.Machine$double.eps)^(1/4))
  varscore_x = varscore_x + ginv(sigmax_hat)
  varscore_x2 = varscore_x2 + ginv(sigmax2_hat)
  
  #observed negative Hessians (joint)
  
  Xstar = cbind(1, X_pred, X2_pred, sp_basisfunctions)
  Y_hess = crossprod(Xstar,iterative_weights)%*%Xstar + 
    bdiag.spam(matrix(0,nrow=3,ncol=3),ginv(sigmarho_hat,1*(.Machine$double.eps)^(1/4)))
  
  X_hess = crossprod(x_basisfunctions)/phi_hat + ginv(sigmax_hat) #X part/block
  X2_hess = crossprod(x2_basisfunctions)/phi_hat2 + ginv(sigmax2_hat)
  
  XY_hess = cbind(fit$par[2]*crossprod(Xstar,iterative_weights)%*%x_basisfunctions,
                  fit$par[3]*crossprod(Xstar,iterative_weights)%*%x2_basisfunctions)
  
  # XY_int_hess = fit$par[2]*apply(iterative_weights%*%x_basisfunctions,2,sum) 
  # XY_slope_hess = fit$par[2]*apply(as.vector(X_pred)*iterative_weights%*%x_basisfunctions,2,sum) #-
  # #as.vector(crossprod(x_basisfunctions, simy - sizes*binomial()$linkinv(eta)))
  # 
  # XY_rho_hess = NULL
  # for (k in 1:ncol(sp_basisfunctions)) {
  #   XY_rho_hess = rbind(XY_rho_hess,
  #                       fit$par[2]*apply(sp_basisfunctions[,k]*iterative_weights%*%x_basisfunctions,2,sum)  )
  # }
  
  #alpha_index = nrow(report$jointPrecision) - ncol(sp_basisfunctions) +1
  #jointPrecision = bdiag(report$jointPrecision[c(1:2,alpha_index:nrow(report$jointPrecision)),c(1:2,alpha_index:nrow(report$jointPrecision))], X_hess)
  
  jointPrecision = bdiag(Y_hess, X_hess, X2_hess)
  jointPrecision[1:(3+ncol(sp_basisfunctions)),(4+ncol(sp_basisfunctions)):ncol(jointPrecision)] =
    XY_hess
  
  out <- list(fit_naive = naive_mod, fit_correct = fit, no.spatial_fit = no.spatial_mod, dnaive_mod = dnaive_mod,
              report = report, varscore = bdiag.spam(varscore_y,varscore_x, varscore_x2), jointPrecision = jointPrecision)
  return(out)
}


true_coefs <- c(-0.1,1,1)
m <- 22500

set.seed(24)
xy_all <- data.frame(x = runif(m), y = runif(m))
#xy_all <- expand.grid(x = ((-sqrt(m)/2):(sqrt(m)/2))[-(sqrt(m)/2+1)], y = ((-sqrt(m)/2):(sqrt(m)/2))[-(sqrt(m)/2+1)])

X2_all <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 0.1), x = xy_all$x, y = xy_all$y, n = 1)@data[,1]

# s = 2*(2*xy_all$x - 1)
# r = 2*(2*xy_all$y - 1)
# X_all <- 2*exp(-(s^2+r^2)) #40 BF ish?

# u_all <- xy_all$x
# v_all <- xy_all$y
# X_all <- 2*(exp(u_all)*(u_all - 0.6)^2*exp(-v_all)*sin(7*v_all)) #50-60 BF ish?

X_all <- RFsimulate(model = RMmatern(nu = 1, var = 0.5, scale = 0.1), x = xy_all$x, y = xy_all$y, n = 1)@data[,1]

#set.seed(NULL)

# u_all <- xy_all$x
# v_all <- xy_all$y
# spranef_all <- 2*(exp(u_all)*(u_all - 0.6)^2*exp(-v_all)*sin(7*v_all)) #50-60 BF ish?

s = 2*(2*xy_all$x - 1)
r = 2*(2*xy_all$y - 1)
spranef_all <- 2*exp(-(s^2+r^2)) #40 BF ish?

#spranef_all <- RFsimulate(model = RMmatern(nu = 0.5, var = 0.5, scale = 0.1), x = xy_all$x, y = xy_all$y, n = 1)@data[,1]

#spranef_all <- rep(0,m)

# x_basisfunctions_all <- mrts(xy_all, k = 150, x = xy_all) %>% 
#   as.matrix
# 
# x2_basisfunctions_all <- mrts(xy_all, k = 100, x = xy_all) %>% 
#   as.matrix
# 
# sp_basisfunctions_all <- mrts(xy_all, k = 5, x = xy_all) %>% 
#   as.matrix
# sp_basisfunctions_all <- sp_basisfunctions_all[,-1] #no intercept


#spranef_all = rep(0,m)

# set.seed(23)
# 
# rho_basisfun_true <- mrts(xy_all, k = 5, x = xy_all) %>%
#   as.matrix
# rho_basisfun_true <- rho_basisfun_true[,-1]
# 
# sigma_rho = diag(1/(1:ncol(rho_basisfun_true)))
# 
# alpha_rho <- rmvnorm(n = 1, mu = rep(0,ncol(rho_basisfun_true)), Sigma = sigma_rho) %>%
#   as.vector
# 
# spranef_all <- rho_basisfun_true%*%alpha_rho
# 
# set.seed(NULL)

reduced_rank <- 1

num_sims <- 2



# allfits = list()
# for (k0 in 1:num_sims) {
#   
#   skip_to_next <- FALSE
#   
#   tryCatch(allfits[[k0]] <- onedataset_sim(k0 = k0), error = function(e) { skip_to_next <<- TRUE})
#   
#   if(skip_to_next) { next } 
#   
#   message(paste0("Finished Dataset ",k0))
# }

summ_tables = list()
big_table = NULL

for (ntest in c(250,500,1000)) {
  for (sigmasq_test in c(0.05,0.1,0.25)) {
    
    sigma_e = sqrt(sigmasq_test)
    
    
    tic <- proc.time()
    allfits <- foreach(i=1:num_sims,.packages = c("MASS","mvtnorm",
                "doParallel","glm2","GGally","Matrix","TMB","dplyr", "autoFRK",
                  "mgcv","spaMM","RandomFields","sp","fields")) %dopar% {
                                                    
        n = ntest
        subn_ind = sample(m,n,replace = TRUE) #
        xy = xy_all[subn_ind,]
        X = X_all[subn_ind]
        X2 = X2_all[subn_ind]
        spranef = spranef_all[subn_ind]
        
        #Generate autoFRK basis functions for fitting
        
        # x_basisfunctions <- x_basisfunctions_all[subn_ind,]
        # 
        # x2_basisfunctions <- x2_basisfunctions_all[subn_ind,]
        # 
        # sp_basisfunctions <- sp_basisfunctions_all[subn_ind,]
        
        x_basisfunctions <- mrts(xy, k = 100, x = xy) %>%
          as.matrix
        
        x2_basisfunctions <- mrts(xy, k = 100, x = xy) %>%
          as.matrix
        
        sp_basisfunctions <- mrts(xy, k = 10, x = xy) %>%
          as.matrix
        sp_basisfunctions <- sp_basisfunctions[,-1] #no intercept
        
        
        
        setwd("D:/Study new/PhD/R code/Spatial_Merror")        
        dyn.load(dynlib("multX_bin"))
        skip_to_next <- FALSE
        tryCatch(result <- onedataset_sim(k0 = i), error = function(e) { skip_to_next <<- TRUE})
        #result <- onedataset_sim(k0 = i)
        if(skip_to_next) { return(NULL) } else { return(result) }
      }
    toc <- proc.time()
    
    
    null_indices = sapply(allfits, function(x) is.null(x))
    allfits <- allfits[!null_indices]
    #save(allfits,file = paste0(sigma_e,"bin_allfits.Rdata"))
    
    ##--------------------------
    ## Process and explore results
    ##-------------------------
    all_naive <- sapply(allfits, function(x) x$fit_naive$coefficients[1:3]) %>%
      t %>%
      as.data.frame
    
    all_correct <- sapply(allfits, function(x) x$fit_correct$par[1:3]) %>%
      t %>%
      as.data.frame
    
    all_no.spatial <- sapply(allfits, function(x) x$no.spatial_fit$coefficients) %>%
      t %>%
      as.data.frame
    
    all_dnaive <- sapply(allfits, function(x) x$dnaive_mod$coefficients) %>%
      t %>%
      as.data.frame
    
    
    naive_var <- sapply(allfits, function(x) summary(x$fit_naive)$coefficients[1:3,2]^2 ) %>%
      t %>%
      as.data.frame
    
    correct_var <- sapply(allfits, function(x){
      invnegHess = as.matrix(solve(x$jointPrecision))
      varcovar = tcrossprod(invnegHess%*%x$varscore,invnegHess)
      c(varcovar[1,1],varcovar[2,2],varcovar[3,3])
    }) %>%
      t %>%
      as.data.frame
    
    no.spatial_var <- sapply(allfits, function(x) summary(x$no.spatial_fit)$coefficients[1:3,2]^2 ) %>%
      t %>%
      as.data.frame
    
    dnaive_var <- sapply(allfits, function(x) summary(x$dnaive_mod)$coefficients[,2]^2 ) %>%
      t %>%
      as.data.frame
    
    
    colnames(all_naive) <- colnames(all_correct) <- colnames(all_dnaive) <-
      colnames(all_no.spatial) <- colnames(naive_var) <- colnames(correct_var) <- 
      colnames(no.spatial_var) <- colnames(dnaive_var) <- c("intercept", "slope", "slope2")
    
    all_coefs <- rbind(all_naive, all_correct, all_no.spatial, all_dnaive) %>%
      mutate(type = rep(c("naive","correct","no_spatial","dnaive"), each = num_sims - sum(null_indices))) %>%
      pivot_longer(intercept:slope2, values_to = "estimate")
    
    
    all_var <- rbind(naive_var, correct_var, no.spatial_var, dnaive_var) %>%
      mutate(type = rep(c("naive","correct","no_spatial", "dnaive"), each = num_sims - sum(null_indices))) %>%
      pivot_longer(intercept:slope2, values_to = "evar")
    
    joined_table = all_coefs %>% mutate(evar = all_var$evar, ssize = ntest, merror = sigmasq_test)
    
    big_table = rbind(big_table,joined_table)
    
    # all_coefs %>% filter(name == "slope") %>%
    #   ggplot(aes(x = type, y = value)) +
    #   geom_boxplot(aes(fill = type)) +
    #   #facet_grid(. ~ name) +
    #   theme_bw() +
    #   geom_hline(aes(yintercept = true_coefs),
    #              data.frame(name = c("intercept","slope"), true_coefs = c(1,1)),linetype="dotted") +
    #   labs(title = "Boxplots", x = 'Procedure' , y = 'Estimate') +
    #   scale_fill_brewer(palette = 1) +
    #   theme(legend.position="right") 
    # 
    # 
    # 
    # all_coefs %>% filter(name == "slope") %>%
    #   ggplot(aes(value)) +
    #   geom_histogram(aes(y=..density..), alpha=0.5, binwidth = 0.25) +
    #   geom_density(col='red',alpha=.2, fill="#FF6666", adjust = 3) +
    #   facet_grid(type ~ .) +
    #   #facet_grid(type ~ name) +
    #   theme_bw() +
    #   labs(title = "Histograms", x = 'Estimated Value' , y = 'Density') +
    #   coord_cartesian(xlim = c(-1,3),ylim = c(0,1))
    
    
    
    empirical_ests = all_coefs %>% 
      group_by(name, type) %>%
      summarise(mean = mean(estimate, na.rm = TRUE), sd = sd(estimate, na.rm = TRUE))
    
    est_vars = all_var %>% 
      group_by(name, type) %>%
      summarise(se = sqrt(mean(evar, na.rm = TRUE)))
    
    summ_table = inner_join(empirical_ests, est_vars, by = c("name","type"))
    #summ_table
    
    summ_tables[[length(summ_tables)+1]] = summ_table
    
    #sapply(allfits, function(x) summary(x$report)[2,2]) %>% mean(na.rm = TRUE)
    
    # sapply(allfits, function(x){
    #   invnegHess = as.matrix(solve(x$jointPrecision))
    #   tcrossprod(invnegHess%*%x$varscore,invnegHess)[2,2]
    # }) %>% mean(na.rm = TRUE) %>% sqrt
    
    
    #sapply(allfits, function(x) summary(x$report)[3,2]) %>% mean(na.rm = TRUE)
    
    # sapply(allfits, function(x){
    #   invnegHess = as.matrix(solve(x$jointPrecision))
    #   tcrossprod(invnegHess%*%x$varscore,invnegHess)[3,3]
    # }) %>% mean(na.rm = TRUE) %>% sqrt
    
    
    
    # sapply(allfits, function(x){
    #   invnegHess = as.matrix(solve(x$jointPrecision))
    #   tcrossprod(invnegHess%*%x$varscore,invnegHess)[2,2]
    # }) %>% as.data.frame %>%
    #   ggplot(aes(x=.)) +
    #   geom_histogram(aes(y=..density..), alpha=0.5, bins = 20, fill = 'blue') +
    #   labs(title = "Histogram", x = 'Estimated Value' , y = 'Density') +
    #   theme_bw()
    # 
    # summ_table %>% as.data.frame %>% xtable(digits = 3)
  }
  
}


#table_full = Reduce(function(...) merge(..., by = c("name","type")), summ_tables)

#save(table_full, file = 'bin_results.Rdata')

#save(big_table, file = 'bin_bigtable.Rdata')

#table_full

stopCluster(cl)

#make sure you do the following steps AFTER the optim() step.

# rep1 <- sdreport(obj)
# rep1 #MLEs
# summary(rep1) #MLEs and also "estimated random effects"
# as.list(rep1,"Est",FALSE) #easier way to look at the results
# as.list(rep1,"Est",TRUE) #reports Sigma here, because of the ADREPORT(Sigma) line in the .cpp file.
# 
# #for some more options when reporting results, see the help files.
