...
TMB::compile("reduced_rank_bin.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("reduced_rank_bin"))

true_coefs = c(-0.1,0.1)
n = 4096
...
x_basisfun_true <- mrts(xy, k = 100, x = xy) %>% 
  as.matrix
...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = TRUE){
  ...
  if (basisfun_truemod) {
    ...
    spranef <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 0.5), x = xy$x, y = xy$y, n = 1)@data[,1] 
  ...
  simy <- rbinom(n, size = 1, prob = binomial()$linkinv(eta))
  

  #Get some 'reasonable' starting values
  
  naive_mod = glm(simy ~ W + sp_basisfunctions , family = binomial)
  no.spatial_mod = glm(simy ~ X_pred , family = binomial)
  ...
  obj <- MakeADFun(data=inputdata,DLL = "reduced_rank_bin",
  ...
  iterative_weights = binomial()$var(binomial()$linkinv(eta)) %>% as.vector %>% diag.spam
  
  for (j in 1:n) {
    varscore_x = varscore_x + tcrossprod(x_basisfunctions[j,]*(W[j] - X_pred[j])/phi_hat ) # - ginv(sigmax_hat)%*%alphax_hat/n
    
    score_y0 = simy[j] - binomial()$linkinv(eta[j]) #-
    ...
    score_y1 = X_pred[j]*(simy[j] - binomial()$linkinv(eta[j])) #- 
    ...
    score_y_rho = sp_basisfunctions[j,]*(simy[j] - binomial()$linkinv(eta[j])) # - ginv(sigmarho_hat)%*%params[-(1:2)]/n
