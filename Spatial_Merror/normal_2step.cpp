...
Type objective_function<Type>::operator() () {  
     ...
     matrix<Type> cholSigmamat(dimSigma,dimSigma); 
     int Sigma_ind = 0;
     for(int l2=0; l2<(dimSigma) ; l2++) { for(int l1=0; l1<(dimSigma); l1++) { 
     ...
     matrix<Type> Sigma = cholSigmamat*cholSigmamat.transpose();
     ADREPORT(Sigma); 
	 ...
	 //data likelihood
	 for(int i=0; i<N; i++) {
		 nll -= dnorm(y(i,0) , eta(i) , Type(1.0) , true);
		  }
