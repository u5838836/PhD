...
TMB::compile("multX_pois.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("multX_pois"))
...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = FALSE){
  ...
  naive_mod = glm(simy ~ W + W2 + sp_basisfunctions , family = poisson)
  no.spatial_mod = glm(simy ~ X_pred + X2_pred , family = poisson)
  dnaive_mod = glm(simy ~ W + W2 , family = poisson)
  ...
  obj <- MakeADFun(data=inputdata,DLL = "multX_pois",
  ...
  for (j in 1:n) {
    ...
    score_y2 = X2_pred[j]*(simy[j] - exp(eta[j]))
...
# u_all <- xy_all$x
# v_all <- xy_all$y
# X_all <- 5*(exp(u_all)*(u_all - 0.6)^2*exp(-v_all)*sin(7*v_all))
...
# u_all <- xy_all$x
# v_all <- xy_all$y
# spranef_all <- 3*(exp(u_all)*(u_all - 0.6)^2*exp(-v_all)*sin(7*v_all)) #50-60 BF ish?
...
#   as.matrix
# 
# sp_basisfunctions_all <- mrts(xy_all, k = 15, x = xy_all) %>% 
#   as.matrix
...
for (ntest in c(250,500,1000)) {
  for (sigmasq_test in c(0.05,0.1,0.25)) {
    ...
    #save(allfits,file = paste0(sigma_e,"pois_allfits.Rdata"))
...
#save(table_full, file = 'pois_results.Rdata')

#save(big_table, file = 'pois_bigtable.Rdata')
