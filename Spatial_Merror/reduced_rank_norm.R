...
#gdbsource("reduced_rank_norm.R",interactive=TRUE)
TMB::compile("reduced_rank_norm.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("reduced_rank_norm"))
...
x_basisfunctions <- mrts(xy, k = 150, x = xy) %>% 
  as.matrix

sp_basisfunctions <- mrts(xy, k = 3, x = xy) %>% 
  as.matrix
sp_basisfunctions <- sp_basisfunctions[,-1] #no intercept

#autoFRK basis functions for the true model

x_basisfun_true <- mrts(xy, k = 150, x = xy) %>% 
  as.matrix

rho_basisfun_true <- mrts(xy, k = 3, x = xy) %>%
  as.matrix
...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = TRUE){
  ...
  if (basisfun_truemod) {
    ...
    #spranef <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 2), x = xy$x, y = xy$y, n = 1)@data[,1] 
  ...
  obj <- MakeADFun(data=inputdata,DLL = "reduced_rank_norm",
