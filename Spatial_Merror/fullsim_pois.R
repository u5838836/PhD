...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = FALSE){
  ...
  out <- list(fit_naive = naive_mod, fit_correct = fit, no.spatial_fit = no.spatial_mod, report = report,
              varscore = bdiag.spam(varscore_y,varscore_x, varscore_x2), jointPrecision = jointPrecision)
  return(out)
}
...
X2_all <- RFsimulate(model = RMmatern(nu = 1, var = 0.5, scale = 5), x = xy_all$x, y = xy_all$y, n = 1)@data[,1]

#set.seed(NULL)

X_all <- 2*exp(-(xy_all$x^2+xy_all$y^2)/sqrt(m))

#X_all <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 5), x = xy_all$x, y = xy_all$y, n = 1)@data[,1]


u_all <- (xy_all$x + 0.5*sqrt(m))/sqrt(m)
v_all <- (xy_all$y + 0.5*sqrt(m))/sqrt(m)

spranef_all <- 5*(exp(u_all)*(u_all - 0.6)^2*exp(-v_all)*sin(7*v_all))
...
for (sigmasq_test in c(0.1)) {
  ...
  allfits <- foreach(i=1:num_sims,.packages = c("MASS","mvtnorm",
      ...
      "mgcv","spaMM","RandomFields","sp","fields")) %dopar% {
        ...
        x_basisfunctions <- mrts(xy, k = 120, x = xy) %>%
          as.matrix

        x2_basisfunctions <- mrts(xy, k = 125, x = xy) %>%
          as.matrix

        sp_basisfunctions <- mrts(xy, k = 15, x = xy) %>%
          as.matrix
  ...
  colnames(all_naive) <- colnames(all_correct) <- 
    ...
    colnames(no.spatial_var) <- c("intercept", "slope", "slope2")
  ...
  all_coefs <- rbind(all_naive, all_correct, all_no.spatial) %>%
    ...
    pivot_longer(intercept:slope2)
  
  
  all_var <- rbind(naive_var, correct_var, no.spatial_var) %>%
    mutate(type = rep(c("naive","correct","no_spatial"), each = num_sims - sum(null_indices))) %>%
    pivot_longer(intercept:slope2)
  ...
  est_vars = all_var %>% 
    group_by(name, type) %>%
    summarise(se = sqrt(mean(value, na.rm = TRUE)))
...
Reduce(function(...) merge(..., by = c("name","type")), summ_tables)
