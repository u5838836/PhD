...
TMB::compile("poisson_structured.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("poisson_structured"))
...
x_basisfunctions <- mrts(xy, k = 3, x = xy) %>% 
  as.matrix

sp_basisfunctions <- mrts(xy, k = 150, x = xy) %>% 
  as.matrix
...
rho_basisfun_true <- mrts(xy, k = 150, x = xy) %>%
  as.matrix
...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = TRUE){
  ...
  parameterlist=list(beta = naive_mod$coefficients[1:2] , delta = delta , 
                     alpha_rho = naive_mod$coefficients[-(1:2)]) # , sigma_nugget_sq = sigma_nugget_sq 
  
  
  obj <- MakeADFun(data=inputdata,DLL = "poisson_structured",
  ...
  varscore_y[-(1:2),-(1:2)] = varscore_y[-(1:2),-(1:2)] + ginv(sigmarho_hat) #,1*(.Machine$double.eps)^(1/4)
  ...
  Y_hess = crossprod(Xstar,iterative_weights)%*%Xstar + 
    bdiag.spam(matrix(0,nrow=2,ncol=2),ginv(sigmarho_hat)) #,1*(.Machine$double.eps)^(1/4)
  ...
  jointPrecision = Y_hess #bdiag(Y_hess, X_hess)
  ...
  out <- list(fit_naive = naive_mod, fit_correct = fit, no.spatial_fit = no.spatial_mod, report = report,
              varscore = varscore_y, jointPrecision = jointPrecision)
  return(out)
}
