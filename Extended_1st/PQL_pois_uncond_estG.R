...
onedatasetsim <- function(i) {     
  ...
  # Generate covariate 
  set.seed(253)
  bigX <- cbind(1,mvrnorm(n = num_clus*num_timepoints,mu = c(0,0),Sigma = matrix(c(0.1,0.05,0.05,0.1),nrow=2)),
  ...
  fit_resps=list()
  for (j in 1:num_resp) {
    fit_resps[[j]] <- glmmTMB(formula = y ~  smallX - 1 + (smallZ-1|ID), family = "poisson",data = subset(simdat, resp == j), 
  ...
  univariate_glmmPQL <- function(starting_fit, resp, max_iter = 10000) {
    ...
    while(err > 1e-5 & counter < max_iter) {  
      ...
      ## G is known
      #new_G = working_G
      new_G = crossprod(new_alpha_mat)/num_clus + diag(rep(0.1,num_Z))
  ...
  for (s in 1:length(timepoint_test)) {
    ...
    for (i in 1:num_sims) {
      ...
      for (j in 1:5) {
        ...
        fixed_CI = c(results[[i]]$unifits[[1]]$beta[j] - 1.96*sqrt(true_G[j,j]/num_clus), 
                     results[[i]]$unifits[[1]]$beta[j] + 1.96*sqrt(true_G[j,j]/num_clus))
        if (true_beta[j]<fixed_CI[2]&true_beta[j]>fixed_CI[1]) {
          assign(paste0("fixedcount",j),get(paste0("fixedcount",j))+1)
        }
        
        
        rand_CI = c(results[[i]]$unifits[[1]]$alpha[1,j] - corrected_quantiles1[2] ,
                    results[[i]]$unifits[[1]]$alpha[1,j] - corrected_quantiles1[1])
        if (results[[i]]$true_alphas[1,j]<rand_CI[2]&results[[i]]$true_alphas[1,j]>rand_CI[1]) {
...
p1 = coverages %>% ggplot(aes(x = clus_size, y = coverage)) +
  ...
  labs(title = "Poisson Responses",x = 'Cluster size' , y = 'Empirical Coverage', col = 'Number of Clusters') + 
...
p1
ggsave("uncond_pois_coverage_estG.pdf", width = 7, height = 7)
...
p2 = PQLshapiros %>% ggplot(aes(x = as.factor(clus_size), y = as.factor(num_clus))) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Poisson Responses",x = 'Cluster size' , y = 'Number of Clusters', fill = 'P-value') + 
  scale_fill_distiller(palette = "Greens")+
  theme(text = element_text(size=15), axis.text.x = element_text(angle = 90,vjust = 0.5))

p2
ggsave("uncond_pois_shapiro_estG.pdf", width = 7, height = 7)
...
p3 = all_estimates %>% 
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Poisson Responses",x = '', y = '') + 
...
p3
ggsave("uncond_pois_fixedhist_estG.pdf", width = 7, height = 7)
...
p4
ggsave("uncond_pois_randhist_estG.pdf", width = 7, height = 7)

frobenius_norm %>% xtable %>% print

diff_norm %>% xtable %>% print

save(diff_norm, file = "uncond_pois_PQLvsLA_estG.Rdata")

save(coverages,PQLshapiros, file = "uncond_pois_estG.Rdata")
