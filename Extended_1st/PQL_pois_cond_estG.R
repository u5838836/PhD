  ...
  for (s in 1:length(timepoint_test)) {
    ...
    for (i in 0:(num_clus-1)) {
      betavar = betavar + solve(crossprod(bigX[(i*num_timepoints+1):((i+1)*num_timepoints),],
                                          diag(exp(eta[(i*num_timepoints+1):((i+1)*num_timepoints)])))%*% 
    ...
    alphavar = solve(crossprod(bigZ[1:num_timepoints,],
                               diag(exp(eta[1:num_timepoints])))%*%bigZ[1:num_timepoints,]/num_timepoints)
...
p1
ggsave("cond_pois_coverage_estG.pdf", width = 7, height = 7)
...
p2
ggsave("cond_pois_shapiro_estG.pdf", width = 7, height = 7)
...
p3
ggsave("cond_pois_fixedhist_estG.pdf", width = 7, height = 7)
...
p4
ggsave("cond_pois_randhist_estG.pdf", width = 7, height = 7)

diff_norm %>% xtable %>% print

save(diff_norm, file = "cond_pois_PQLvsLA_estG.Rdata")

save(coverages,PQLshapiros, file = "cond_pois_estG.Rdata")
