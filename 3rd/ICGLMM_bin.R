...
onedatasetsim <- function(i) {     
  ...
  fit_glmer <- glmer(formula = y ~  x_int + x_slope -1 + (z_int + z_slope - 1|ID), family = "binomial",data = simdat)
...
for (r in 1:length(clus_test)) {
  for (s in 1:length(timepoint_test)) {
    ...
    #save(results,true_beta,true_G,file=paste0(num_clus,num_timepoints,"bin_uncond.Rdata"))
...
# bin_coverage = rbind(fixedcoverage1,fixedcoverage2,randomcoverage1,randomcoverage2,lcombcoverage1,lcombcoverage2)
# save(bin_coverage, file="bin_coverage.Rdata")
# print(xtable(bin_coverage,digits=3), include.rownames=FALSE)

bin_coverage_TMB = rbind(TMB_randomcoverage1,TMB_randomcoverage2,TMB_lcombcoverage1,TMB_lcombcoverage2) #TMB_fixedcoverage1,TMB_fixedcoverage2,
#save(bin_coverage_TMB, file="bin_coverage_TMB.Rdata")
print(xtable(bin_coverage_TMB,digits=3), include.rownames=FALSE)

bin_coverage_lme = rbind(lme_randomcoverage1,lme_randomcoverage2,lme_lcombcoverage1,lme_lcombcoverage2) #lme_fixedcoverage1,lme_fixedcoverage2,
#save(bin_coverage_lme, file="bin_coverage_lme.Rdata")
print(xtable(bin_coverage_lme,digits=3), include.rownames=FALSE)

bin_corrected_coverage = rbind(corrected_randomcoverage1,corrected_randomcoverage2,lcombcoverage1,lcombcoverage2)
#save(bin_corrected_coverage, file="bin_coverage_corrected.Rdata")
print(xtable(bin_corrected_coverage,digits=3), include.rownames=FALSE)

# bin_shapiro = rbind(fixedPQLshapiro1,fixedPQLshapiro2,randomPQLshapiro1,randomPQLshapiro2,diffsPQLshapiro1,diffsPQLshapiro2,
...
# save(bin_shapiro, file="bin_shapiro.Rdata")
# print(xtable(bin_shapiro,digits=3), include.rownames=FALSE)
