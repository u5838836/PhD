...
PQL_G_error = rep(0,num_sims)
LA_G_error = rep(0,num_sims)
...
univariate_glmmPQL <- function(starting_fit, max_iter = 10000) {
  ...
  cw_G <- starting_fit$fit$parfull[length(fit_resp$fit$parfull)]
  ...
  while(err > 1e-7 & counter < max_iter) {  
    ...
    update_beta <- glm2(simdat$y ~ 1, offset = make_offset, family = "poisson")
    ...
    bigW <- as.vector(poisson()$var(exp(new_beta + make_offset)))
    cw_G0 <- cw_G
    err2 <- 1
    while(err2 > 0.01) {
      new_G <- sapply(1:num_clus, function(j) {
        selindices <- (num_timepoints*(j-1)+1):(num_timepoints*j)
        solve(sum(bigW[selindices]) + solve(cw_G0)) + tcrossprod(new_alpha_mat[j,])
      })
      new_G <- matrix(mean(new_G), nrow = 1)
...
for (j in 1:num_sims) {
  ...
  fit_resp <- glmmTMB(formula = y ~ 1 + (1|ID), family = "poisson",data=simdat)
  
  sum(fit_resp$fit$parfull[-c(1,length(fit_resp$fit$parfull))])/true_G #this should be equal to the first derivative of the logdet term since X_i=Z_i
  
  dummy_Z = matrix(0,nrow = num_clus*num_timepoints,ncol = num_clus)
  for (i in 1:num_clus) {
    dummy_Z[((i-1)*num_timepoints+1):(i*num_timepoints),i] = 1
  }
  mu_est = exp(fit_resp$fit$parfull[1] + dummy_Z%*%fit_resp$fit$parfull[-c(1,length(fit_resp$fit$parfull))]) #estimated means for each obv
  
  sum(simdat$y - mu_est) - sum(fit_resp$fit$parfull[-c(1,length(fit_resp$fit$parfull))])/true_G #this is correctly very close to 0
  ...
  LA_sum_pred_error[j] = (sum(fit_resp$fit$parfull[-c(1,length(fit_resp$fit$parfull))]) - sum(true_alpha))^2 #Square prediction error of Laplace for sum of b_i
  PQL_sum_pred_error[j] = sum(true_alpha)^2 #Square prediction error of PQL for sum of b_i
  LA_vec_pred_error[j] = sum((fit_resp$fit$parfull[-c(1,length(fit_resp$fit$parfull))] - true_alpha)^2)
  ...
  LA_G_error[j] = abs(exp(fit_resp$fit$par[2])^2 - true_G)
  PQL_G_error[j] = abs(PQL_fit$G - true_G)
}
