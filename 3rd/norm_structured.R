...
#gdbsource("norm_structured.R",interactive=TRUE)
TMB::compile("norm_structured.cpp","-O1 -g",DLLFLAGS="")
dyn.load(dynlib("norm_structured"))
...
x_basisfunctions <- mrts(xy, k = 10, x = xy) %>% 
  as.matrix
...
x_basisfun_true <- mrts(xy, k = 3, x = xy) %>% 
  as.matrix

rho_basisfun_true <- mrts(xy, k = 10, x = xy) %>%
  as.matrix
rho_basisfun_true <- rho_basisfun_true[,-1] #no intercept
...
scalar_x = 3
sigma_x = scalar_x*diag(ncol(x_basisfun_true))
...
scalar_rho = 3
sigma_rho = scalar_rho*diag(ncol(rho_basisfun_true))
...
onedataset_sim = function(k0, basisfun_truemod = FALSE , stationary_x = TRUE){
  ...
  if (basisfun_truemod) {
    ...
    if (stationary_x) {
      X <- RFsimulate(model = RMmatern(nu = 1.5, var = 0.5, scale = 5), x = xy$x, y = xy$y, n = 1)@data[,1]
  ...
  simy <- eta + rnorm(n, sd = sigma_y)
  
  
  #Get some 'reasonable' starting values
  
  naive_mod = lm(simy ~ W + sp_basisfunctions)
  no.spatial_mod = lm(simy ~ X_pred)
  ...
  parameterlist=list(beta = naive_mod$coefficients[1:2] , scalar_rho = scalar_rho , 
                     alpha_rho = naive_mod$coefficients[-(1:2)])
  
  
  obj <- MakeADFun(data=inputdata,DLL = "norm_structured",
  ...
  for (j in 1:n) {
    ...
    score_y1 = X_pred[j]*(simy[j] - eta[j]) #- 
    ...
    score_y_rho = sp_basisfunctions[j,]*(simy[j] - eta[j]) # - ginv(sigmarho_hat)%*%params[-(1:2)]/n
  ...
  varscore_y[-(1:2),-(1:2)] = varscore_y[-(1:2),-(1:2)] + ginv(sigmarho_hat)
  ...
  Y_hess = crossprod(Xstar,iterative_weights)%*%Xstar + 
    bdiag.spam(matrix(0,nrow=2,ncol=2),ginv(sigmarho_hat))
